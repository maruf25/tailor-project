...
const Order = () => {
  ...
  return (
    <section
      className="flex flex-col items-center justify-center gap-3 p-6 text-black lg:p-16"
      id="order"
    >
      <AnimatePresence>
        {showImage && <PopUpImage image={image} onShow={() => setShowImage(false)} />}
      </AnimatePresence>
      <h1 className="text-xl font-bold md:text-3xl md:mb-0">Order</h1>
      {orders.length !== 0 ? (
        <div className="grid grid-cols-2 gap-4 md:gap-8 md:grid-cols-4">
          {orders.map((order, key) => (
            <div key={key} className="relative max-w-sm mx-auto shadow-lg">
              <div className="p-4 bg-white rounded-lg shadow">
                <div className="relative flex justify-center h-24 overflow-hidden bg-gray-600 rounded-lg md:h-32 lg:h-52">
                  <div
                  ...
                  <span className="absolute top-0 left-0 z-10 inline-flex px-1 py-1 mt-1 ml-1 text-sm font-medium text-white bg-yellow-500 rounded-lg select-none md:ml-3 md:mt-3 md:px-3 md:py-2">
                    Proses
                  </span>
                </div>
                ...
                <div className="mt-2 md:mt-4">
                  <h2
                    className="text-base font-medium text-gray-800 md:text-lg line-clamp-1 text-ellipsis"
                    title={order.name}
                  >
                    {order.name}
                  </h2>
                  <p
                    className="mt-2 text-xs text-gray-800 line-clamp-1 text-ellipsis"
                    title={moment(order.dateline).format("dddd, MM/YYYY")}
                  >
                    Deadline: {moment(order.dateline).format("dddd, DD MMMM YYYY")}
                  </p>
                </div>

                <div className="hidden grid-cols-2 grid-rows-2 gap-4 mt-8 lg:grid">
                  <p className="inline-flex flex-col text-gray-800 xl:flex-row xl:items-center">
                    ...
                    <span className="mt-2 xl:mt-0">{order.size_pinggang} Cm</span>
                  </p>
                  <p className="inline-flex flex-col text-gray-800 xl:flex-row xl:items-center">
                    <svg
                      className="inline-block w-5 h-5 mr-3 text-gray-800 fill-current xl:w-4 xl:h-4"
                ...
                <div className="grid-cols-2 mt-8 lg:grid">
                  <div className="flex items-center">
                    <p
                      className="text-xs text-gray-800 md:text-base lg:ml-2 line-clamp-1 text-ellipsis"
                  ...
                  <div className="flex items-center lg:justify-end">
                    <p className="inline-block font-semibold md:leading-tight text-primary whitespace-nowrap rounded-xl">
                      <span className="text-xs md:text-lg">
                        {formatRupiah(order.price.toString())}
                      </span>
                    </p>
                  </div>
                </div>
        ...
        <div className="grid grid-cols-1">
          <h1 className="text-3xl font-bold">Sedang Tidak Ada Orderan</h1>
        </div>
      )}
    </section>
  );
